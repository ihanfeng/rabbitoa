...
<script type="text/javascript">
      ...
      function showDeptLimitsPanel()
      {
      	...
      	if(gr)
		{
			...
			$("#user-dialog").html($("<iframe />").attr("name","menusPanel").attr("src", "${cpath}/limitsManager/listByDeptid?deptid="+gr).attr("width","99%").attr("height","100%").attr("frameBorder","0")).dialog('open');
		}
		else
		{
			alert('请选择一行再进行操作！');
